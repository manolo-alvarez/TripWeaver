...
## Get prompt files ##
init_itinerary_prompt_file = open("ref/itinerary_prompt.txt", "r")
init_itinerary_prompt = init_itinerary_prompt_file.read()
init_itinerary_prompt_file.close()
init_score_prompt_file = open("ref/score_prompt.txt", "r")
init_score_prompt = init_score_prompt_file.read()
init_score_prompt_file.close()
cities_file = open("ref/cities.txt", "r")
cities = cities_file.readlines()
cities_file.close()
dates_file = open("ref/dates.txt", "r")
dates = dates_file.readlines()
dates_file.close()
######################}

# Open data file
data_file = open('data/itineraries_2.json', 'w')
...
for i in range(len(dates)):
  ...
  score_1_response = client.chat.completions.create(
    ...
    temperature=1.0, # want a deterministic score
    messages=[
      {"role": "user",
