...
#model_name = '././models/dpo/beta_0.1/final_merged_checkpoint'
...
tokenizer = LlamaTokenizer.from_pretrained('meta-llama/Llama-2-7b-hf')#././models/dpo/beta_0.1/final_checkpoint')
tokenizer.bos_token_id = 1

stop_token_ids = [0]
model.eval()

prompt = "I will be traveling to Austin, United States from 2025-07-15 through 2025-07-17. Generate an itinerary that details activities, transportation between destinations, approximate expenses per activity, breakfast location, lunch location, dinner location, is flexible, and doesn't cost more than $1130 total (not including flights and accommodation). Limit your response to 700 words."
...
def generate(model, max_new_tokens=max_new_tokens, top_p=top_p, temperature=temperature):
    ...
    with open('././tmp/itinerary_llama2-7b_Austin_temp-1.0_top-p-1.0_new.txt', 'w') as f:
        f.write(text)
